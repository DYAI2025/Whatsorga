#!/bin/bash
# Test script for GET /api/response-times/{chat_id}
...
echo "=========================================="
echo "Testing Response Times Endpoint"
echo "=========================================="
...
echo "Test 1: GET /api/response-times/${CHAT_ID} (default 30 days)"
echo "------------------------------------------"
response=$(curl -s -w "\nHTTP_STATUS:%{http_code}" \
  "${BASE_URL}/api/response-times/${CHAT_ID}" \
...
echo "Test 2: GET /api/response-times/${CHAT_ID}?days=7"
echo "------------------------------------------"
response=$(curl -s -w "\nHTTP_STATUS:%{http_code}" \
  "${BASE_URL}/api/response-times/${CHAT_ID}?days=7" \
  -H "Authorization: Bearer ${API_KEY}")
